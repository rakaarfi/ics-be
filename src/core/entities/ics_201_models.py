...
class Ics201(SQLModel, table=True):
    ...
    actions_strategies_tactics: List["ActionsStrategiesTactics"] = Relationship(
...
class ActionsStrategiesTactics(SQLModel, table=True):
    ...
    ics_201: Optional["Ics201"] = Relationship(
        back_populates="actions_strategies_tactics"
    )
...
class IcsChart(SQLModel, table=True):
    ...
    compensation_claim_unit_leader: Optional["CompensationClaimUnitLeader"] = (
        Relationship(back_populates="ics_chart")
    )
