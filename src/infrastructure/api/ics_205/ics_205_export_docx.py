...
from src.core.entities.ics_205_models import (
    Ics205,
    Ics205Preparation,
    Ics205RadioChannel
)
...
async def export_docx(id: int, session: AsyncSession = Depends(get_session)):
    template_path = "src/infrastructure/templates/ics_205.docx"

    # Ambil data Ics201
    ics_205 = await read_item_by_id(Ics205, item_id=id, session=session)
    context = ics_205.model_dump()
    ics_205_id = ics_205.id
    ...
    communication_unit_leader_id = ics_205_preparations[0].communication_unit_leader_id
    prepared_name = await read_item_by_id(CommunicationUnitLeader, item_id=communication_unit_leader_id, session=session)
    context["prepared_name"] = prepared_name.name
    context['is_prepared'] = '✓' if ics_205_preparations[0].is_prepared else '✗'
    context['date_prepared'] = format_date_to_sentence(ics_205_preparations[0].date_prepared.strftime("%Y-%m-%d"))
    context['time_prepared'] = ics_205_preparations[0].time_prepared.strftime("%H:%M")
    ...
    return StreamingResponse(
        ...
        headers={"Content-Disposition": f"attachment; filename=ics_205_{id}.docx"}
    )
