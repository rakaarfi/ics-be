...
from src.core.entities.ics_209_models import (
    Ics209,
    Ics209Preparation,
    Ics209Approval
)
from src.core.entities.imt_members_models.main_section_models import IncidentCommander
from src.core.entities.imt_members_models.planning_section_models import SituationUnitLeader
...
async def export_docx(id: int, session: AsyncSession = Depends(get_session)):
    template_path = "src/infrastructure/templates/ics_209.docx"
    ...
    situation_unit_leader_id = ics_209_preparations[0].situation_unit_leader_id
    prepared_name = await read_item_by_id(SituationUnitLeader, item_id=situation_unit_leader_id, session=session)
    context["prepared_name"] = prepared_name.name
    context["is_prepared"] = "✓" if ics_209_preparations[0].is_prepared else "✗"
    context['date_prepared'] = format_date_to_sentence(ics_209_preparations[0].date_prepared.strftime("%Y-%m-%d"))
    context['time_prepared'] = ics_209_preparations[0].time_prepared.strftime("%H:%M")
    ...
    return StreamingResponse(
        ...
        headers={"Content-Disposition": f"attachment; filename=ics_209_{id}.docx"}
    )
