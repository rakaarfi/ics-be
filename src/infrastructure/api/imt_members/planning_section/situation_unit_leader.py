...
async def create_situation_unit_leader(
    item: SituationUnitLeader, repo: BaseRepository = Depends(get_repository)
):
    return await repo.create_item(SituationUnitLeader, item)

# Endpoint untuk read
@router.get("/read/")
async def read_situation_unit_leader(repo: BaseRepository = Depends(get_repository)):
    return await repo.read_items(SituationUnitLeader)
...
async def read_situation_unit_leader_by_id(
    ...
    try:
        return await repo.read_item_by_id(SituationUnitLeader, id)
...
async def update_situation_unit_leader(
    ...
    try:
        return await repo.update_item(SituationUnitLeader, id, update_data)
    except NotFoundException as e:
        raise e
...
async def delete_situation_unit_leader(
    ...
    try:
        return await repo.delete_item(SituationUnitLeader, id)
    except NotFoundException as e:
        raise e
...
async def read_situation_unit_leader_paginated(
    ...
    return await repo.read_paginated_items(
        SituationUnitLeader, page, limit, condition
    )
