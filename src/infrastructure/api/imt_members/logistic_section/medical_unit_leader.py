...
async def create_medical_unit_leader(
    item: MedicalUnitLeader, repo: BaseRepository = Depends(get_repository)
):
    return await repo.create_item(MedicalUnitLeader, item)


# Endpoint untuk read
@router.get("/read/")
async def read_medical_unit_leader(repo: BaseRepository = Depends(get_repository)):
    return await repo.read_items(MedicalUnitLeader)
...
async def read_medical_unit_leader_by_id(
    ...
    try:
        return await repo.read_item_by_id(MedicalUnitLeader, id)
...
async def update_medical_unit_leader(
    ...
    try:
        return await repo.update_item(MedicalUnitLeader, id, update_data)
    except NotFoundException as e:
        raise e
...
async def delete_medical_unit_leader(
    ...
    try:
        return await repo.delete_item(MedicalUnitLeader, id)
    except NotFoundException as e:
        raise e
...
async def read_medical_unit_leader_paginated(
    ...
    return await repo.read_paginated_items(MedicalUnitLeader, page, limit, condition)
