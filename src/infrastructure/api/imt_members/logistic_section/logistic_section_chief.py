...
from src.core.entities.imt_members_models.logistic_section_models import LogisticSectionChief
...
async def create_logistic_section_chief(
    item: LogisticSectionChief, repo: BaseRepository = Depends(get_repository)
):
    return await repo.create_item(LogisticSectionChief, item)


# Endpoint untuk read
@router.get("/read/")
async def read_logistic_section_chief(repo: BaseRepository = Depends(get_repository)):
    return await repo.read_items(LogisticSectionChief)
...
async def read_logistic_section_chief_by_id(
    ...
    try:
        return await repo.read_item_by_id(LogisticSectionChief, id)
...
async def update_logistic_section_chief(
    ...
    try:
        return await repo.update_item(LogisticSectionChief, id, update_data)
    except NotFoundException as e:
        raise e
...
async def delete_logistic_section_chief(
    ...
    try:
        return await repo.delete_item(LogisticSectionChief, id)
    except NotFoundException as e:
        raise e
...
async def read_logistic_section_chief_paginated(
    ...
    return await repo.read_paginated_items(
        LogisticSectionChief, page, limit, condition
    )