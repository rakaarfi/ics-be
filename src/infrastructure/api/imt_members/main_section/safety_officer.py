...
async def create_safety_officer(
    item: SafetyOfficer, repo: BaseRepository = Depends(get_repository)
):
    return await repo.create_item(SafetyOfficer, item)


# Endpoint untuk read
@router.get("/read/")
async def read_safety_officer(repo: BaseRepository = Depends(get_repository)):
    return await repo.read_items(SafetyOfficer)
...
async def read_safety_officer_by_id(
    ...
    try:
        return await repo.read_item_by_id(SafetyOfficer, id)
...
async def update_safety_officer(
    update_data: SafetyOfficer, id: int, repo: BaseRepository = Depends(get_repository)
):
    try:
        return await repo.update_item(SafetyOfficer, id, update_data)
    except NotFoundException as e:
        raise e
...
async def delete_safety_officer(id: int, repo: BaseRepository = Depends(get_repository)):
    try:
        return await repo.delete_item(SafetyOfficer, id)
    except NotFoundException as e:
        raise e
...
async def read_safety_officer_paginated(
    ...
    return await repo.read_paginated_items(SafetyOfficer, page, limit, condition)
