...
from app.models.incident_data import OperationalPeriod, IncidentData
...
@router.get(
    "/read-paginated/",
    response_model=PaginationResponse[List[OperationalPeriod]]
)
async def read_operational_period_with_pagination(
    ...
    if search:
        ...
        condition = (
            OperationalPeriod.incident_id.in_(subquery) |
            func.lower(func.to_char(OperationalPeriod.date_from, 'YYYY-MM-DD')).like(search_lower) |
            func.lower(func.to_char(OperationalPeriod.time_from, 'HH24:MI')).like(search_lower) |
            func.lower(func.to_char(OperationalPeriod.date_to, 'YYYY-MM-DD')).like(search_lower) |
            func.lower(func.to_char(OperationalPeriod.time_to, 'HH24:MI')).like(search_lower) |
