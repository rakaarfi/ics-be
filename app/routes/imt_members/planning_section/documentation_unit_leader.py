...
from app.models.imt_members_models.planning_section_models import DocumentationUnitLeader
...
async def create_documentation_unit_leader(
    item: DocumentationUnitLeader, session: AsyncSession = Depends(get_session)
):
...
async def read_documentation_unit_leader_paginated(
    ...
    return await read_paginated_items(
        DocumentationUnitLeader, page, limit, session, condition
    )
