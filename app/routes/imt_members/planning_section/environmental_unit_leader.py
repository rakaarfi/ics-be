...
from app.models.imt_members_models.planning_section_models import EnvironmentalUnitLeader
...
async def create_environmental_unit_leader(
    item: EnvironmentalUnitLeader, session: AsyncSession = Depends(get_session)
):
...
async def read_environmental_unit_leader_paginated(
    ...
    return await read_paginated_items(
        EnvironmentalUnitLeader, page, limit, session, condition
    )
