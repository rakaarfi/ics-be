...
from app.models.imt_members.logistic_section import CommunicationUnitLeader
...
# Endpoint untuk read
@router.get("/read/")
async def read_communication_unit_leader(
    session: AsyncSession = Depends(get_session)
):
...
@router.get("/read-paginated/", 
            response_model=PaginationResponse[List[CommunicationUnitLeader]])
async def read_communication_unit_leader_with_pagination(
    ...
    return await read_paginated_items(CommunicationUnitLeader, page, limit, session, condition)