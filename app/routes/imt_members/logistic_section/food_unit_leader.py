...
from app.models.imt_members_models.logistic_section_models import FoodUnitLeader
...
async def create_food_unit_leader(
    item: FoodUnitLeader, session: AsyncSession = Depends(get_session)
):
...
async def update_food_unit_leader(
    update_data: FoodUnitLeader, id: int, session: AsyncSession = Depends(get_session)
):
