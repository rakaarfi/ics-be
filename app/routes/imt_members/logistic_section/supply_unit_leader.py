...
from app.models.imt_members_models.logistic_section_models import SupplyUnitLeader
...
async def create_supply_unit_leader(
    item: SupplyUnitLeader, session: AsyncSession = Depends(get_session)
):
...
async def update_supply_unit_leader(
    update_data: SupplyUnitLeader, id: int, session: AsyncSession = Depends(get_session)
):
...
async def read_supply_unit_leader_paginated(
    ...
    return await read_paginated_items(SupplyUnitLeader, page, limit, session, condition)
