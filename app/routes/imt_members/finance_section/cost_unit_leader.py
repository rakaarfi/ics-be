...
from app.models.imt_members_models.finance_section_models import CostUnitLeader
...
async def create_cost_unit_leader(
    item: CostUnitLeader, session: AsyncSession = Depends(get_session)
):
...
async def update_cost_unit_leader(
    update_data: CostUnitLeader, id: int, session: AsyncSession = Depends(get_session)
):
