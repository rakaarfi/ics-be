...
from app.models.imt_members_models.finance_section_models import TimeUnitLeader
...
async def create_time_unit_leader(
    item: TimeUnitLeader, session: AsyncSession = Depends(get_session)
):
...
async def update_time_unit_leader(
    update_data: TimeUnitLeader, id: int, session: AsyncSession = Depends(get_session)
):
