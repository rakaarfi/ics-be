...
from app.models.imt_members_models.finance_section_models import ProcurementUnitLeader
...
async def create_procurement_unit_leader(
    item: ProcurementUnitLeader, session: AsyncSession = Depends(get_session)
):
...
async def read_procurement_unit_leader_paginated(
    ...
    return await read_paginated_items(
        ProcurementUnitLeader, page, limit, session, condition
    )
