...
from app.models.imt_members_models.finance_section_models import FinanceSectionChief
...
async def create_finance_section_chief(
    item: FinanceSectionChief, session: AsyncSession = Depends(get_session)
):
...
async def read_finance_section_chief_paginated(
    ...
    return await read_paginated_items(
        FinanceSectionChief, page, limit, session, condition
    )
