...
from app.models.imt_members.finance_section import CompensationClaimUnitLeader
...
async def create_compensation_claim_unit_leader(
    item: CompensationClaimUnitLeader, 
...
# Endpoint untuk read
@router.get("/read/")
async def read_compensation_claim_unit_leader(session: AsyncSession = Depends(get_session)):
...
async def read_compensation_claim_unit_leader_paginated(
    ...
    return await read_paginated_items(CompensationClaimUnitLeader, page, limit, session, condition)