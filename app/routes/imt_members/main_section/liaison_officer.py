...
from app.models.imt_members_models.main_section_models import LiaisonOfficer
...
async def create_liaison_officer(
    item: LiaisonOfficer, session: AsyncSession = Depends(get_session)
):
...
async def update_liaison_officer(
    update_data: LiaisonOfficer, id: int, session: AsyncSession = Depends(get_session)
):
...
async def delete_liaison_officer(id: int, session: AsyncSession = Depends(get_session)):
