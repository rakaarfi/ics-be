...
async def create_imt_roster(
    item: ImtRoster, session: AsyncSession = Depends(get_session)
):
...
@router.get("/read/{id}")
async def read_imt_roster_by_id(id: int, session: AsyncSession = Depends(get_session)):
    return await read_item_by_id(ImtRoster, id, session)


@router.put("/update/{id}")
async def update_imt_roster(
    update_data: ImtRoster, id: int, session: AsyncSession = Depends(get_session)
):
    return await update_item(ImtRoster, id, update_data, session)


@router.delete("/delete/{id}")
async def delete_imt_roster(id: int, session: AsyncSession = Depends(get_session)):
...
async def read_imt_roster_paginated(
    ...
    return await read_paginated_items(ImtRoster, page, limit, session, condition)
