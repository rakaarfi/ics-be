...
from app.models.ics_201 import ResourceSummary
...
async def update_resource_summary(
    updated_data: ResourceSummary,
    id: int,
    session: AsyncSession = Depends(get_session)
):
    return await update_item(
        ResourceSummary, id, updated_data, session
    )
...
async def delete_resource_summary_by_ids(session, ids):
    await session.execute(
        delete(ResourceSummary).where(ResourceSummary.id.in_(ids))
    )
    await session.commit()
    
@router.delete("/delete-many/")
async def delete_multiple_resource_summary(
    ids: ResourceSummaryDelete,
    session: AsyncSession = Depends(get_session)
):
...
@router.get(
    "/read-paginated/",
    response_model=PaginationResponse[List[ResourceSummary]]
)
async def read_resource_summary_paginated(
    ...
    if search:
        ...
        condition = (
                ...
                func.lower(func.to_char(ResourceSummary.date_ordered, 'YYYY-MM-DD')).like(search_lower) |
                func.lower(func.to_char(ResourceSummary.time_ordered, 'HH24:MI')).like(search_lower) |
