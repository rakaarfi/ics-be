...
from sqlmodel import func, Field, SQLModel, Time, select
...
from app.models.ics_201 import ActionsStrategiesTactics
...
@router.post(
    "/create/", 
    summary="Create Actions Strategies Tactics", description="Create a new Actions Strategies Tactics"
)
async def create_actions_strategies_tactics(
    item: ActionsStrategiesTacticsCreate, 
...
# Endpoint untuk read
@router.get("/read/")
async def read_actions_strategies_tactics(
    session: AsyncSession = Depends(get_session)
):
    return await read_items(
        ActionsStrategiesTactics, session
    )
...
async def read_actions_strategies_tactics_by_id(
    ...
    return await read_item_by_id(
        ActionsStrategiesTactics, id, session
    )
...
async def update_actions_strategies_tactics(
    ...
    return await update_item(
        ActionsStrategiesTactics, id, updated_data, session
    )
...
async def delete_multiple_actions(
    ids: ActionsStrategiesTacticsDelete,
    session: AsyncSession = Depends(get_session)
):
...
@router.get(
    "/read-paginated/",
    response_model=PaginationResponse[ActionsStrategiesTactics]
)
async def read_actions_strategies_tactics_paginated(
    ...
    if search:
        ...
        condition = (
                func.lower(ActionsStrategiesTactics.actions).like(search_lower) |
                func.lower(func.to_char(ActionsStrategiesTactics.time_initiated, 'HH24:MI')).like(search_lower)
            )
...
@router.get(
    ...
    response_model=List[ActionsStrategiesTactics])
async def read_actions_strategies_tactics_by_ics_id(
    ics_201_id: int,
    session: AsyncSession = Depends(get_session)
):
    # Query the database for records with the specified ics_201_id
    statement = select(ActionsStrategiesTactics).where(ActionsStrategiesTactics.ics_201_id == ics_201_id)
