...
async def create_incident_data(
    item: IncidentData, session: AsyncSession = Depends(get_session)
):
...
async def update_incident_data(
    update_data: IncidentData, id: int, session: AsyncSession = Depends(get_session)
):
...
async def delete_incident_data(id: int, session: AsyncSession = Depends(get_session)):
...
async def read_incident_data_with_pagination(
    ...
    return await read_paginated_items(IncidentData, page, limit, session, condition)
